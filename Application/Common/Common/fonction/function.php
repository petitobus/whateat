 ...
 * @return mixed
 */
function lang($name=null, $file = null, $vars = array()) {
    static $_lang = array();
    $path = LANG_PATH.config('DEFAULT_LANG').'/';
    if($file) {
        if(strpos($file, '/') !== false) {
            list($module, $file) = explode('/', $file);
            $path = APP_PATH.config('DEFAULT_H_LAYER').'/'.$module.'/language/'.config('DEFAULT_LANG').'/';
        }
        if(is_file($path.$file.'.php')) $array = include $path.$file.'.php';
        if($array) $_lang = array_merge($_lang, array_change_key_case($array, CASE_UPPER));
    }
	...
	if($vars && is_array($vars)) {
		$vars = array_key_case($vars);
		foreach($vars as $k => $v) {
			$searchs[] = '{'.$k.'}';
			$replaces[] = $v;
		}
	}
	$_lang = str_replace($searchs, $replaces, $_lang);
...
function model($name, $layer = 'table') {
    return core::load_model($name, $layer);
}
...
function pages($totalrow, $pagesize = 10, $pagenum = 5) {
	...
	$string .= '<li class="last">共<b>'.$totalPage.'</b>页&nbsp;到第<input nmae="page" class="input" type="text" value="1">页&nbsp;<a class="button bg-gray-white" href="#">确定</a></li>';
    $string .= '</ul>';
    return $string;
}
...
function is_zipcode($string){
     if (!empty($string)) {
          return preg_match('/^[1-9][0-9]{5}$/', $string);
     }
     return FALSE;
}
